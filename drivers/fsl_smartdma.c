...
void SMARTDMA_Boot(uint32_t apiIndex, void *pParam, uint8_t mask)
{
	...
	SMARTDMA0->PENDTRAP = (0xFF << 16);//enable pending trap
...
/**
* @brief Trigger SmartDMA operation on specific bit
* @param bit: Bit position to trigger (0-7)
*/
void SmartDMA_software_trigger_bit(uint8_t bit)
{
	if (bit < 8) {  // Validate bit range (0-7)
		// Generate trigger pulse: set then clear bit
		SMARTDMA0->PENDTRAP |= (1U << bit);
		SMARTDMA0->PENDTRAP &= ~(1U << bit);
	}
}

/**
* @brief Trigger SmartDMA operation on multiple bits
* @param bits: Bit mask for trigger channels
*/
void SmartDMA_software_trigger_bits(uint8_t bits)
{
	// Generate trigger pulse: set then clear bits
	SMARTDMA0->PENDTRAP |= bits;
	SMARTDMA0->PENDTRAP &= ~bits;
}
